...
以下包装是 Proof-Script DSL 策略。在 `:= script` 中，用户应使用不带前缀的策略名；框架会自动将
其改写为内部执行入口 `script_<name>`，录制时再使用 `_script_<name>`。普通 `:= by` 证明不能使用
这些脚本策略（包括 `script_<name>` 和 `_script_<name>`），应直接使用 Mathlib 的原生策略名。
...
Lean `by` 证明中直接使用 Mathlib 原生策略，并保持原生策略名不带 `script_` 前缀。
